...
class JsonString {
 public:
  ...
  JsonString* at(Rcpp::List path) {
    ...
    return new JsonString(jsonXptr(new json(js), false));
  }
  ...
  JsonString* patch(jsonXptr jspatchptr) {
    ...
    try {
      ...
      return new JsonString(jsonXptr(new json(jsresult), false));
